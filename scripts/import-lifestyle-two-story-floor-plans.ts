...
import {
  buildLifestyleTwoStoryPlanDescription,
  getLifestyleTwoStoryPlanSlug,
  LIFESTYLE_TWO_STORY_STYLE,
  lifestyleTwoStoryPlanSeeds
} from "../lib/lifestyle-two-story-floor-plan-data";
...
function getPreviewPath(slug: string, floor: "first" | "second"): string {
  return path.join(process.cwd(), "public", "floor-plans", "lifestyle-two-story", "previews", `${slug}-${floor}.png`);
}

function getPdfPath(slug: string): string {
  return path.join(process.cwd(), "public", "floor-plans", "lifestyle-two-story", "pdfs", `${slug}.pdf`);
}
...
async function importLifestyleTwoStoryFloorPlans() {
  console.log(`Importing Lifestyle Two Story floor plans into ${projectId}/${dataset}...`);
  ...
  const styleDoc = styles.find((style) => style.slug === LIFESTYLE_TWO_STORY_STYLE.slug);
  if (!styleDoc) {
    throw new Error(`Missing floorPlanStyle with slug ${LIFESTYLE_TWO_STORY_STYLE.slug}`);
  }

  const existingPlanBySlug = new Map(plans.map((plan) => [plan.slug, plan]));

  for (const [index, seed] of lifestyleTwoStoryPlanSeeds.entries()) {
    const slug = getLifestyleTwoStoryPlanSlug(seed);
    ...
    await client.createOrReplace({
      ...
      baths: seed.baths,
      sqFt: 2000,
      description: buildLifestyleTwoStoryPlanDescription(seed),
      mainImage: {
        _type: "image",
        asset: {
          _type: "reference",
    ...
    console.log(`Upserted Lifestyle Two Story plan: ${seed.name}`);
  }

  console.log(`Done. Imported ${lifestyleTwoStoryPlanSeeds.length} Lifestyle Two Story entries.`);
}

importLifestyleTwoStoryFloorPlans().catch((error) => {
  console.error("Lifestyle Two Story import failed:", error);
  process.exit(1);
});
